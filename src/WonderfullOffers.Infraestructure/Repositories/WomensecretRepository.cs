...
public class WomensecretRepository : CompanyRepository<WomensecretOfferEntity>, IWomensecretRepository
{
    public WomensecretRepository(CompanyContext companyContext)
        : base(companyContext) { }

    public async Task<DateTime> GetLastTimeSpanAsync()
    {
        return await GetLastTimeSpanAsync(women => women.TimeSpan);
    }

    public async Task<List<IOfferEntity>> GetOffersAsync(int numberPaginationFrontEnd)
    {
        List<IOfferEntity> womenOffers = await GetOffersAsync(
            women => women.Disccount,
